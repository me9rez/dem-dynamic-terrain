...
import { reprojectImage, getBuildOverviewResampling } from './gdal-util';
import { tileBoundMap, ST_TileEnvelope, getTileByCoors, type Bound } from './tile-util';
import type { Dataset, Driver } from 'gdal-async'
import type { Options, StatisticsInfo, OverviewInfo, CreateInfo } from './type'
...
export let statistics: StatisticsInfo = {
  tileCount: 0,
  completeCount: 0,
  levelInfo: {}
}
 ...
 * 重投影数据集
 */
const reproject = (ds: Dataset, epsg: number, resampling: number) => {
  let projectDatasetPath = path.join(os.tmpdir(), 'dem-dynamic-terrain', `${uuid()}.tif`);
...
async function generateTile(input: string, output: string, options: Options) {
  ...
  // xyz是从左上角开始，往右下角走
  let startPoint: [number, number] = [dsInfo.startX, maxy!];
  let endPoint: [number, number] = [dsInfo.endX!, miny!];
